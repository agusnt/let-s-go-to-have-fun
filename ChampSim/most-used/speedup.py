...
def get_from_db(db, filter):
    ...
    for i in workloads:
        all = {}
        bench = {}
        for ii in collection.find({'workload': i}, {filter, 'name', 'speedup', 'binary'}):
            if ii[filter] == False: continue

            bin = ii['binary']
            if bin not in all: all[bin] = []
            if ii['name'] not in bench: bench[ii['name']] = {}

            # Add the data
            bench[ii['name']][ii['binary']] = ii['speedup']
            all[bin].append(ii['speedup'])

        # Get the geomean
        for ii in all: all[ii] = gmean(all[ii])
        yield i, all, bench
...
if __name__ == '__main__':
    ...
    allG = {}
    for workload, all, toCSV in get_from_db(json.load(open(sys.argv[1])), config['filter']):
    ...
    print("Gen fig")
    y, x, lg = gen_dat_for_graph(allG, config)
    gen_graph(config, y, x, lg, config['output'])
